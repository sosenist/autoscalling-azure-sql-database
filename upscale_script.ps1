...
if ($WebhookData){
    ...
    if (($status -eq "Activated") -or ($status -eq "Fired"))
    {
        ...
        $max_rsob = $currentDatabaseDetails.Tags['max_rsob']
        Write-Output "max_rsob: $max_rsob"
        ...
        if (($currentDatabaseDetails.Edition -eq "Basic") -Or ($currentDatabaseDetails.Edition -eq "Standard") -Or ($currentDatabaseDetails.Edition -eq "Premium"))
        {
            ...
            if ($currentDatabaseDetails.CurrentServiceObjectiveName -eq $max_rsob) {
                Write-Output "DTU database is already at highest tier configured for this resource"
            } else {
