...
if ($WebhookData){
    ...
    if (($status -eq "Activated") -or ($status -eq "Fired"))
    {
        ...
        $min_rsob = $currentDatabaseDetails.Tags['min_rsob']
        Write-Output "min_rsob: $min_rsob"
        ...
        if (($currentDatabaseDetails.Edition -eq "Basic") -Or ($currentDatabaseDetails.Edition -eq "Standard") -Or ($currentDatabaseDetails.Edition -eq "Premium"))
        {
            ...
            if ($currentDatabaseDetails.CurrentServiceObjectiveName -eq $min_rsob) {
            ...
            if ($currentVcores -eq $coresArrayToBeUsed[$coresArrayToBeUsed.length]) {
                Write-Output "vCore database is already at lowest number of cores. Suggestion is to optimize workload."
            } else {
    ...
    Update-AzAlertState -AlertId $Essentials.alertId -State "Closed"
}
